...
public class LogHelper {
    ...
    private static final long MAX_LOG_SIZE_BYTES = Config.getInt("Logger.maxSize", 52428800); // 50 MiB
			...
			if (logFiles != null && logFiles.length > maxLogs) {
				...
				for (int i = 0; i < filesToDelete; i++) {
					try {
						...
						Logger.debug("Log antiguo borrado: " + logFiles[i].getName());
					} catch (IOException e) {
						Logger.error("Error borrando log " + logFiles[i].getName() + ": " + e.getMessage());
					}
				}
			}
		} catch (Exception e) {
    ...
    public static void createLog() {
        try {
            ...
            String logDir = Config.configDir + CARPETA_LOGS;
